...
- **Habitat page's map doesn't render / blank grey box** → make sure `npm install` completed successfully (it needs to pull `leaflet` + `react-leaflet`) and that you have internet access in the browser tab — the map tiles themselves load live from OpenStreetMap.
- **Population / Habitat / Conservation / Ecosystem Health pages show "no data yet"** → run `python -m app.seed` (safe to re-run) — it now also creates a demo survey, 5 GPS sites, and ~30 species-labeled observations. See `MILESTONE4_CHANGES.md` for full details on what changed for Milestone 3 & 4 and why.

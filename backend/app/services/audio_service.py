...
# Milestone 4 Performance Metrics (FR-8): the load+resample step below is
# pure local work (librosa) that needs no network access, unlike the full
# YAMNet inference which can be blocked in sandboxed environments (see
# module docstring). Timing it real-time gives an honest, always-available
# "audio processing latency" figure even when the YAMNet weights fetch
# itself is unavailable - see performance_service.py.
_last_audio_preprocess_ms: float | None = None


def get_last_audio_preprocess_ms() -> float | None:
    return _last_audio_preprocess_ms
...
def classify_animal_sound(file_path: str) -> dict:
    ...
    global _last_audio_preprocess_ms
    _last_audio_preprocess_ms = round((time.perf_counter() - _preprocess_start) * 1000, 2)

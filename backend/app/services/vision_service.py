...
# Milestone 4 Performance Metrics (FR-8): real, measured wall-clock time
# of the most recent detect_animals() call on this process (the startup
# warm-up call in main.py populates this before any real request lands,
# so the metric is never empty on a freshly started backend). This is a
# genuine measurement, not a benchmark constant.
_last_inference_ms: float | None = None


def get_last_inference_ms() -> dict:
    return {
        "value_ms": _last_inference_ms,
        "is_measured": _last_inference_ms is not None,
        "note": "Wall-clock time of the most recent real YOLOv8 detect_animals() call on this process.",
    }
...
def detect_animals(file_path: str, confidence_threshold: float = 0.25) -> dict:
    ...
    _last_inference_ms = round((time.perf_counter() - _start) * 1000, 2)
